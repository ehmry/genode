			...
			case PIPE_CONTROL:
				{
					...
					if (!(ctrl->bRequestType & USB_DIR_IN))
						_usb.source()->apply_payload(_packet, [&] (char *payload, size_t) {
							Genode::memcpy(payload, _urb.transfer_buffer,
			...
			case PIPE_BULK:
				{
					...
					if (usb_pipeout(_urb.pipe))
						_usb.source()->apply_payload(_packet, [&] (char *payload, size_t) {
						Genode::memcpy(payload, _urb.transfer_buffer, _urb.transfer_buffer_length); });
					return;
				}
			...
			if (packet.succeded) {
				...
				if (_urb.actual_length && _urb.transfer_buffer &&
				    (_urb.transfer_buffer_length >= _urb.actual_length))
						_usb.source()->apply_payload(_packet, [&] (char *payload, size_t) {
							Genode::memcpy(_urb.transfer_buffer, payload,
							               _urb.actual_length); });
			}

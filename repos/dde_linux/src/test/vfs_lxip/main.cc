...
static void ls_socket_fs(char const *path)
{
	DIR *dp = opendir(path);
	if (dp == NULL) {
		perror("opendir");
...
{
	char const sock_root[] = "/socket";

	ls_socket_fs(sock_root);

	int fd = open("/socket/new_socket", O_RDONLY);
	if (fd == -1) {
		perror("open");
		return -1;
	}
	...
	char sock_fd[8];
	size_t n = read(fd, sock_fd, sizeof(sock_fd));
	sock_fd[n-1] = '\0';
	close(fd);
	...
	test_bind_accept(sock_root, sock_fd);
	// test_connect_recv(sock_root, sock_fd);

	ls_socket_fs(sock_dir);

	remove_sock_dir(sock_root, sock_fd);

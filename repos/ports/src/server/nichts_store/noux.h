...
namespace Nichts_store {
	...
	Builder::startNoux()
	{
		 ...
		 * place.
		 */
	build_dir = nix::createTempDir("", "nix-build-" + nix::storePathToName(drvPath), false, false, 0700);
		...
		char* config_ds_addr =
			Genode::env()->rm_session()->attach(config_ds);
		Genode::Xml_generator xml(config_ds_addr, CONFIG_SIZE, "config", [&]
		{
			...
			xml.node("start", [&] {
				...
				*/
				 ...
				 */
				env_node(xml, "NIX_STORE", settings.nixStore);

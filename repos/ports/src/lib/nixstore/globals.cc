...
Settings::Settings()
{
    ...
    fsyncMetadata = true;
    useSQLiteWAL = false; /* Not without locking support in SQLite. */
    syncBeforeRegistering = false;
    useSubstitutes = true; /* Substitutes unsupported for now. */
    useChroot = false;
...
static string get_config_string(Genode::Xml_node node, char const *key, char const *default_value)
{
    if (node.has_attribute(key))
    {
        Genode::Xml_node::Attribute attribute = node.attribute(key);
        char cstr[attribute.value_size()];
        attribute.value(cstr, sizeof(cstr));
        return string(cstr);
    }
    else
...
static long get_config_integer(Genode::Xml_node node, const char *attr, long default_value)
{
	long x = default_value;
	try { Genode::config()->xml_node().attribute(attr).value(&x); }
	catch (...) { }
	return x;
}
...
{
	/* TODO: drop settings that are not required */
	Genode::Xml_node node = Genode::config()->xml_node();

	buildTimeout = get_config_integer(node, "build-timeout", 0);

	nixStore = canonPath(get_config_string(node, "store-dir", "/nix/store"));
	nixDataDir = canonPath(get_config_string(node, "data-dir", "/nix/data"));
	nixLogDir = canonPath(get_config_string(node, "log-dir", "/nix/log"));
	nixStateDir = canonPath(get_config_string(node, "state-dir", "/nix/state"));
	nixDBPath = get_config_string(node, "database", (nixStateDir + "/db").c_str());
	nixConfDir = canonPath(get_config_string(node, "conf-dir", "/nix/conf"));
}

...
/* Acquire the global GC lock.  This is used to prevent new Nix
   ...
   yielded the GC lock. */
int Store::openGCLock(LockType lockType)
{
	static Path fnGCLock("/nix/state/gc.lock");
...
void Store::syncWithGC()
{
    AutoCloseFD fdGCLock = openGCLock(ltRead);
}


void Store::addIndirectRoot(const Path & path)
{
	string hash = printHash32(hashString(htSHA1, path));
	Path realRoot = ((format("/nix/state/gcroots/auto/%1%")
		% hash).str());
	create_symlink(realRoot.c_str(), path.c_str());
}
...
bool Store::isActiveTempFile(const GCState & state,
...
void Store::deleteGarbage(GCState & state, const Path & path)
{
...
void Store::deletePathRecursive(GCState & state, const Path & path)
{
    checkInterrupt();
...
bool Store::canReachRoot(GCState & state, PathSet & visited, const Path & path)
{
...
void Store::tryToDelete(GCState & state, const Path & path)
{
    checkInterrupt();
...
/* Unlink all files in /nix/store/.links that have a link count of 1,
   ...
   the link count. */
void Store::removeUnusedLinks(const GCState & state)
{
...
void Store::collectGarbage(const GCOptions & options, GCResults & results)
{

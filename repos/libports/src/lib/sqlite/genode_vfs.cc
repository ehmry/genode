 ...
 * number times 86400000. In other words, write into *piNow
...
static int genode_current_time(sqlite3_vfs *pVfs, double *pTime)
{
	...
	*pTime = (julian_day(ts)+0.5) * 86400000.0;
	*pTime += ts.hour   * 360000.0;
	*pTime += ts.minute *  60000.0;
	*pTime += ts.second *   1000.0;
	/* *pTime +=  ts.microsecond / 1000.0; */
	/* Use the timer session for milisecond granularity. */

...
class Fatfs::File_system : public Vfs::File_system
{
	private:
		 ...
		 */
		//Genode::Timeout<Fatfs::File_system> _flush_timeout {
		...
		void _notify(File &file)
		{
			...
			{
				/* unmark all handles to avoid redundant notification */
				handle->modifying = false;
				if (handle->watching)
					_io_handler.handle_io_response(handle->context, CONTENT_CHANGED);
			}
		...
		void close(Vfs_handle *vfs_handle) override
		{
			...
			if (file) {
				...
				if (!file->handles.first()) {
					_close(*file);
				} else if (handle->modifying) {
					/* this handle modified its file, send notification */
					_notify(*file);
				}
			}
...
struct Ffat_factory : Vfs::File_system_factory
{
	struct Inner : Vfs::File_system_factory
	{
		...
		{ 
			return new (alloc)
				Fatfs::File_system(env, alloc, node, io_handler);
		}
	};

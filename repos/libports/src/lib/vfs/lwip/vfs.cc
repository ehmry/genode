			...
			case Lwip_file_handle::DATA:
				{
					...
					_recv_off += n;
					{
						...
						if (new_head != NULL && new_head != _recv_pbuf) {
							...
							/* call realloc to adjust the "tot_len" field on the new head */
							u16_t new_len = (_recv_pbuf->tot_len - _recv_off) + new_off;
							pbuf_realloc(new_head, new_len);

							/* free the buffers skipped from the old head */

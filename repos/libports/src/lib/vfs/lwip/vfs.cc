...
struct Lwip::Socket_dir
{
		...
		virtual bool notify_read_ready(Lwip_handle const &handle) = 0;

		void handle_io(Lwip::Lwip_handle_list &list)
		{
			if (Lwip::Lwip_handle *h = list.first())
	...
	public Udp_socket_dir_list::Element
{
	...
	public:
		...
		// TODO old
		bool notify_read_ready(Lwip_handle const &handle) override {
		                   ...
		                   file_size &out_count) override
		{
			...
			case Type::REMOTE:
					...
					return Write_result::WRITE_ERR_INVALID;
				{
					...
					Genode::strncpy(buf, src, min(sizeof(buf)-1, count));
			...
			case Type::BIND:
				if ((state == NEW) && (count < ENDPOINT_STRLEN_MAX)) {
					...
					Genode::strncpy(buf, src, sizeof(buf));
			...
			case Type::CONNECT:
				if (((state == NEW) || (state == BOUND)) && (count < ENDPOINT_STRLEN_MAX)) {
					char buf[ENDPOINT_STRLEN_MAX];
	...
	public Tcp_socket_dir_list::Element
{
	...
	public:
		...
		{
			if (_pcb == NULL) {
				Genode::error("socket closed");
				out_count = 0;
				/* TODO: return zero or error? */
				return Write_result::WRITE_ERR_INVALID;
			}
...
class Lwip::File_system final : public Vfs::File_system
{
	...
	public:
		...
		Read_result read(Vfs_handle *vfs_handle,
		                 ...
		                 file_size &out_count) override
		{
			Read_result out_result = _read(vfs_handle, dst, count, out_count);
			if (out_result == Read_result::READ_QUEUED)
				return Read_result::READ_ERR_WOULD_BLOCK;
			return out_result;
		}
		...
		virtual bool notify_read_ready(Vfs_handle *vfs_handle)
		{
			if (Lwip_handle *handle = dynamic_cast<Lwip_handle*>(vfs_handle)) {
				if (handle->socket) {
					if (handle->socket->notify_read_ready(*handle)) {
						handle->notify = true;
						return true;
					}
				}
		                                      ...
		                                      Signal_context_capability sigh)
		{
			if (Lwip_handle *handle = dynamic_cast<Lwip_handle*>(vfs_handle)) {
				...
				Genode::error(__func__," not registered");
		}

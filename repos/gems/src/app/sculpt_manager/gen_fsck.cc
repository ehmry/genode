/*
 * \brief  XML configuration for file-system check
 * \author Norman Feske
 * \date   2018-05-02
 */
...
void Sculpt_manager::gen_fsck_start_content(Xml_generator &xml,
                                            Block_device::Label const &device,
                                            Partition::Number const &partition)
{
	gen_common_start_content(xml, String<64>(device, ".", partition, ".check"),
	...
	xml.node("config", [&] () {
		...
		xml.node("start", [&] () {
			xml.attribute("name", "/bin/fsck.ext2");

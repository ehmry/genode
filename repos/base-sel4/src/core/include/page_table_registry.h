...
class Genode::Page_table_registry
{
	...
	private:
		...
		class High : public Avl_node<High>
		{
			private:
				...
				High *_lookup(addr_t vaddr)
				{
					if (vaddr == _vaddr) return this;

					High *e = Avl_node<High>::child(vaddr > _vaddr);
			...
			public:
				...
				High(addr_t const vaddr, addr_t const paddr,
				...
				static High * lookup(Avl_tree<High> &tree,
				                     addr_t const vaddr,
				                     unsigned const log2base)
				{
					High * element = tree.first();
					if (!element)
						return nullptr;
				...
				bool higher(High const *other) const {
					return other->_vaddr > _vaddr; }
		};
		...
		Tslab<High, SLAB_BLOCK_SIZE> _alloc_high;
		uint8_t _initial_sb_high[SLAB_BLOCK_SIZE];

		Avl_tree<Frame> _frames;
		Avl_tree<High>  _level1;
		Avl_tree<High>  _level2;
		Avl_tree<High>  _level3;
		             ...
		             addr_t const paddr, unsigned const level_log2_size)
		{
			try {
				...
				case FRAME:
					...
					break;
				case PAGE_TABLE:
					_level1.insert(new (_alloc_high) High(vaddr, paddr, sel,
					                                         level_log2_size));
					break;
				case LEVEL2:
					_level2.insert(new (_alloc_high) High(vaddr, paddr, sel,
					                                      level_log2_size));
					break;
				case LEVEL3:
					_level3.insert(new (_alloc_high) High(vaddr, paddr, sel,
					                                      level_log2_size));
					break;
				}
	...
	public:
		...
		bool page_table_at(addr_t const vaddr, addr_t const level_log2) {
			return High::lookup(_level1, vaddr, level_log2); }
		bool page_directory_at(addr_t const vaddr, addr_t const level_log2) {
			return High::lookup(_level2, vaddr, level_log2); }
		bool page_level3_at(addr_t const vaddr, addr_t const level_log2) {
			return High::lookup(_level3, vaddr, level_log2); }

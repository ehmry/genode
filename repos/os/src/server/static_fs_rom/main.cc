/*
 * \brief  Component that provides static ROM sessions from a file-system
...
class Static_fs_rom::Rom_file final
{
	private:
	...
	public:
		...
		void process_packet(File_system::Packet_descriptor const packet)
		{
			...
			if (_file_seek >= _file_size) {
				...
				/* attached dataspace into read-only region map */
				enum { OFFSET = 0, LOCAL_ADDR = 0, EXEC = 0, WRITE = 0 };
				_rm_attachment = _rm.attach(
					_file_ds.cap(), _file_size, OFFSET,
					LOCAL_ADDR, LOCAL_ADDR, EXEC, WRITE);
...
class Static_fs_rom::Session_component final : public  Rpc_object<Rom_session>
{
	private:
...
struct Static_fs_rom::Main final : Genode::Session_request_handler
{
	Genode::Env &env;
	                           ...
	                           Session_state::Args const &args) override
	{
		...
		if (!rom_file) {
			...
			/* eviction failed */
			error("insufficient resources for '", label, "'"
...
{
	static Static_fs_rom::Main inst(env);

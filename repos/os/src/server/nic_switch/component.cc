                                   ...
                                   Size_guard     &size_guard)
{
	...
	if (arp.ethernet_ipv4() &&
		...
		Ipv4_address_node *node = vlan().ip4_tree.first();
		if (node)
...
bool Session_component::handle_ip4(Ethernet_frame &eth,
                                   Size_guard     &size_guard)
{
...
{
	Ipv4_address_node * first = vlan().ip4_tree.first();
	if (!first) return;
	if (first->find_by_address(_ipv4_node.addr()))
		vlan().ip4_tree.remove(&_ipv4_node);
}
...
void Session_component::set_ipv4_address(Ipv4_address ip_addr)
{
	...
	vlan().ip4_tree.insert(&_ipv4_node);
}

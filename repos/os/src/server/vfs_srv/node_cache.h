...
#ifndef _VFS_SRV__NODE_CACHE_H_
#define _VFS_SRV__NODE_CACHE_H_
 ...
 * into an AVL tree.
 */
struct File_system::Node : Node_base, Avl_path_string
{
	unsigned _ref_count;

	Node(char const *path) : Avl_path_string(path), _ref_count(0) { }
...
struct File_system::Directory : Node
{
	...
	size_t read(char *dst, size_t len, seek_off_t seek_offset)
	{
		...
		while (remains >= blocksize) {
			...
			if (root()->dirent(name(), index++, vfs_dirent)
...
class File_system::File : public Node
{
	...
	public:
		...
		void open(Mode fs_mode)
		{
			...
			assert_open(root()->open(name(), mode, &new_handle));
...
struct File_system::Symlink : Node
{
	...
	{
		Vfs::file_size res = 0;
		root()->readlink(name(), dst, len, res);
		return res;
	}

	size_t write(char const *src, size_t len, seek_off_t seek_offset)
	{
		len = max(size_t(MAX_PATH_LEN-1), len);

		char from[MAX_PATH_LEN];
		strncpy(from, src, sizeof(from));

		root()->unlink(name());
		size_t n = (root()->symlink(from, name()) == Directory_service::SYMLINK_OK)
			? len : 0;
 ...
 */
struct File_system::Node_cache : Avl_tree<Avl_string_base>
{
	Node *find(char const *path) {
		return first() ? (Node *)first()->find_by_name(path) : nullptr; }

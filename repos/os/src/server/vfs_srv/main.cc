...
class File_system::Session_component : public Session_rpc_object
{
	...
	public:
		...
		:
			...
			_writable(writable)
		{
			strncpy(_root_path, root_path, sizeof(_root_path));
		...
		Dir_handle dir(Path const &path, bool create) override
		{
			...
			/* re-root the path */
			Subpath dir_path(path_str+1, _root_path);
		                 ...
		                 Mode fs_mode, bool create) override
		{
			...
			/* re-root the path */
			Subpath subpath(name_str, dir->name());
		...
		Node_handle node(Path const &path) override
		{
			...
			/* re-root the path */
			Subpath sub_path(path_str+1, _root_path);
		...
		void unlink(Dir_handle dir_handle, Name const &name)
		{
			...
			Subpath path(name_str, dir->name());
			_handle_registry.remove(path.base());
			assert_unlink(root()->unlink(path.base()));
			dir->mark_as_updated();
		}
		          ...
		          Dir_handle to_dir_handle, Name const &to_name)
		{
			...
			if (_handle_registry.refer_to_same_node(from_dir_handle, to_dir_handle)) {
				...
				from_str = Subpath(from_str, dir->name()).base();
				  to_str = Subpath(  to_str, dir->name()).base();

				assert_rename(root()->rename(from_str, to_str));
				_handle_registry.remove(from_str);
				dir->mark_as_updated();
				return;
			}
			...
			from_str = Subpath(from_str, from_dir->name()).base();
			  to_str = Subpath(  to_str,   to_dir->name()).base();
		...
		/**
		 * Send any pending signal on the node and sync the VFS.
		 */
		void sync(Node_handle handle)
		{
			...
			/* would be nice to only sync the FS that the node is on */
			root()->sync();
		}
...
char const *   Server::name() { return "vfs_srv_ep"; }

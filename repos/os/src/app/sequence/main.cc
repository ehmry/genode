...
struct Sequence::Child : Genode::Child_policy
{
	...
	char const *_config_addr() const
	{
		try { return _start_node.sub_node("config").addr(); }
		catch (...) { return nullptr; }
	}

	size_t _config_size() const
	{
		try { return _start_node.sub_node("config").size(); }
		catch (...) { return 0; }
	}

	Genode::Static_rom_session _config_session {
		_env.ep().rpc_ep(), _env.pd(), _env.rm(),
		_config_addr(), _config_size() };

...
struct File_system::Session : public Genode::Session
{
	 ...
	 * \throw Out_of_ram         server cannot allocate metadata
	 * \throw Out_of_caps
	 * \throw Permission_denied  file-system is static or does not support
	 *                           notifications
	 *
	 * The returned node handle is used to distinguish node context on
	 * notification packets. 
	 */
	...
	GENODE_RPC_THROW(Rpc_watch, Watch_handle, watch,
	                 GENODE_TYPE_LIST(Lookup_failed, Out_of_ram, Out_of_caps, Permission_denied),
	                 Path const &);
	...
	GENODE_RPC_INTERFACE(Rpc_tx_cap, Rpc_file, Rpc_symlink, Rpc_dir,
	                     Rpc_node, Rpc_watch,

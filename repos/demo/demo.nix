...
let
  ...
  contents = with pkgs;
    ...
    { target = "/genode/launchpad.config";
  ...
  hydraProductsScript = builtins.toFile "hydra-products.sh" ''
    mkdir -p $out/nix-support
    echo $PATH
    cat > $out/nix-support/hydra-build-products <<EOF
    $(find $systemImage -printf 'file binary-dist %p\n')
    file iso $diskImage
    EOF
  '';
...
derivation {
  ...
  builder = tool.shell;
  args = [ "-e" hydraProductsScript ];
  PATH="${tool.nixpkgs.coreutils}/bin:${tool.nixpkgs.findutils}/bin";

  inherit systemImage diskImage;
}
